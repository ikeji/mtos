...
line_buf__SourceReader__StringBuffer:
    # prologue: frame_size=48, params=2, locals=6
    addi sp, sp, -48
    sw   ra, 44(sp)
    sw   s0, 40(sp)
    addi s0, sp, 48
...
lex_tok__SourceReader__U8Array:
  ...
  .L_f59_pc2:
    ...
    sw   t0, 0(sp)
    lw   a0, 0(sp)
    call line_buf_reset__SourceReader
    addi sp, sp, 4
    addi sp, sp, -4
    sw   a0, 0(sp)
    addi sp, sp, 4
