...
- [x] スクリプトが増えすぎて把握できなくなった。それぞれの意味、どれがどれを呼ぶか、などをドキュメントにまとめて。
...
- [x] neofetchを作る
- [x] ls /procが失敗する。
- [x] cat 存在しないファイル がエラーにならない。
- [x] sh にタブ補完つけて。
- [x] viコマンド作って。
- [x] ls / を実行したときに/procや/tmpがでてこない。
- [x] pico2_tty.pyが次の行に行くとき行頭に行かない。
- [x] /procに、空きメモリとか、cpu情報とか、クロックカウントとか入れて。
- [x] /proc/tasksにヘッダ行をつけて。
- [x] /proc/tasksに起動中のファイル名を出して。
- [x] /proc/tasksにタスクが使ってるメモリ量ram/stackを教えて。
- [x] shにヒストリをつける（上下矢印で直近8件を呼び出し）。
- [x] タブ補完で複数候補に共通プレフィックスがある場合、そこまで補完する。
- [x] kernel/build.shを解体したい。毎回ディスクイメージを作りなおしてる。フルに全ての環境が入ったイメージと、テストに必要な最小のイメージの両方を使い必要に応じて使う。
- [x] make runで、hello, hello2, shが同時に起動するが、shのstdinからの読み込みIOが残り2つをブロックしている。
- [x] pipeをサポートして欲しい。
