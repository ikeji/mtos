...
| ファイル | 説明 |
|---|---|
| `mkfs.tc` | MyTinyFS ディスクイメージ生成。フェーズ 8 (2026-05-06) で `tools/mkfs.py` を TC port、`build/gen2/mkfs` (RV32 ELF + qemu-riscv32) で kernel build が呼ぶ。byte-exact 一致 (Python の inode-pack 60 byte バグも忠実に再現)。statx (291) でパス stat (qemu-riscv32 user は newfstatat / fstat 未実装) |
