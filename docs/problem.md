...
### K2. スケジューラが固定スロット数 (limitation)

`kernel_common.tc` の `sched_init(n)` で `n` スロットを用意し、
`sched_register(idx, frame)` で埋めるだけ。動的な task 生成 (fork /
spawn) には対応していない。

フェーズ6 Step 6.5 で `sched_spawn(frame) -> slot_id` と state 拡張
(ready/done/unused/waiting) を追加する予定。
