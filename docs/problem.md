...
### 7. `String` と `StringLiteral` の分離でオーバーロード爆発 (limitation)

`append(s: String, t: String)`, `append(s: String, t: StringLiteral)`,
`append(s: StringLiteral, t: String)`, `append(s: StringLiteral, t: StringLiteral)`
のように 2^N 個の組み合わせを用意する必要がある。詳細は
`docs/task/string_type_split.md`。

対処案:
- `StringLiteral <: String` の暗黙変換ルールを入れる。

---

## asm.tc (アセンブラ兼リンカ)

### 8. `.macro` / `.endm` 未対応 (limitation)

アセンブラマクロが使えないため、trap_common.s のように
ecall ハンドラを書くときに prologue/epilogue をコピペすることになる。
一箇所直すと全部直す必要がある。

### 9. `.align 12` 未対応 (limitation)
...
### 10. エラーメッセージが `asm: unknown instruction 'xxx'` だけ (ergonomics)

行番号が出ない。入力行は連結された .s ファイルで、grep する手がかりが
ほぼ命令名だけになる。

### 11. struct array 呼び出しが mangle 名で未解決 (FIXED)
...
### 12. bcrun.tc の `shr_u` 実装が手動マスク (limitation)
...
### 13. `poke8` / `poke16` / `poke32` は境界チェックなし (by design)

`set(U8Array, i, v)` は範囲チェックするが、raw アドレスへの poke は
しない。これは意図的 (MMIO とカーネル用) だが、ユーザコードで使う
と簡単にメモリを壊せる。

### 14. Compiler bug: struct 定義時にも auto-generated array の size が u32 ハードコード
...
### 15. `compile-gen2.sh` が stderr を `2>/dev/null` で潰している (ergonomics)
...
### 16. `/tmp/gen2` キャッシュが stale になる (ergonomics)
...
### 17. Gen2 ツールの再ビルドが遅い (ergonomics)
...
### 18. `make test` の実行時間が 60 秒上限にぎりぎり (ergonomics)
