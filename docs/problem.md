...
### 1. u32 の `/` と `%` が signed 除算になっている (bug)
...
u32 比較の修正 (`lt_u` 系) と同じ構造で潰せる。

### 2. 16 進リテラルの top-bit (`0x80000000u32` 等) が壊れる (bug)

lexer が `ival * 16 + d` で i32 のまま累積するので、0x80000000 のように
最上位ビットが立つ値は途中で signed overflow する。0x80000000 単独は
bit-pattern が INT32_MIN と同じなので偶然復元できるが、それ以上の値
(`0xFFFFFFFF` など) は累積オーバーフローで破損する。

影響:
- `addr >= 0xFFFF0000u32` 等のコードが書けない (現状のワークアラウンド:
  `(1u32 << 31) | (0x7FFF0000u32)` のように分割構築する)。
- 2025-04 時点で compiler/bcrun.tc が `1 << 31` を使っているのはこれの
  回避。

対処案:
- lexer で hex/binary の累積を u64 (または 64-bit) に移す。
- もしくは parse.tc 側で suffix u32/u16/u8 を見て unsigned 型で parse する。
