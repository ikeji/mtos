# prelude pre-encode（asm_pass3 の prelude 処理を撲滅する案）

## 動機

phase 7 self-host で **asm_pass3 が 53 秒**（全体 57 秒の 93%）。
内訳の大半は prelude.s（~12000 行）を毎回 token + dispatch +
encode する処理。**prelude は kernel build 時点で固定**なので、
事前に raw バイナリへ encode し、OS 側 asm_pass3 はそれを memcpy
するだけにすれば大半の仕事が消える。

**期待値**: asm_pass3 53 s → ~5-10 s、TOTAL 57 s → **~15 s**。
...
kernel/tasks/task_data.s                                │
                              ...
                              │ prelude.idx                  │ ◀─── asm_pass2 --emit-idx prelude.s
...
/hw.tc ──parse──▶ /sd/1.ast ──sigscan──▶ /sd/1.th
        ...
        /sd/4.s + /prelude_tail.s ─[cat]─▶ /sd/u.s ◀────────────────┘
                                              │
                                              ▼  asm_pass2 --load-idx /prelude.idx
...
`prelude.idx` は asm_pass2 の **per-file 中間状態**。
`asm_pass3` は `lab.s` の `src` 行に従って `/prelude.s`（mtfs/XIP）と
`/sd/u.strip`（SD）を直接 open し、3 回 walk して text/rodata/data
の bytes を emit。

ここで asm_pass3 が prelude.s を 3 回 walk するのが重い（~50 s）。

**注**: 2026-05-01 に tcheck が file-args (`--exth` / `--tgth` /
`--tgt` / `--out`) 対応になり cat-wrap step が不要になった
(commit 775a004)。同じ file-args パターンを **asm_pass2 / asm_pass3
にも適用するのが pre-encode の前提**：asm_pass3 が複数の入力ファイル
...
─────────────────────────────────────────────────────────────
prelude.s ── asm_pass2 + asm_pass3 (host) ──▶ prelude.text.bin    (~50 KB)
...
未定。リンク時 (asm_pass3 OS-side) に target を resolve して当該
...
asm_pass3 は `src raw` を memcpy、`src asm` を encode、`reloc` を最後に patch。
...
| **`asm_pass2 --emit-idx`** | prelude.s | **prelude.idx**（既存） |
| **`asm_pass2 --emit-bin`** *(新)* | prelude.s | **prelude.{text,rodata,data}.bin + prelude.reloc**（新） |
| `mkfs.py` | staging dir | mtfs.img |

`asm_pass2 --emit-bin` モードを新設：
- pass 1a で label 収集
...
| **asm_pass2** | **/prelude.idx + /sd/u.s** | **/sd/lab.s + /sd/u.strip**（変更：lab に src raw 行を埋め込み） |
| **asm_pass3** *(改造)* | **/sd/lab.s + /prelude.{text,rodata,data}.bin + /prelude.reloc + /sd/u.strip** | **/sd/HW** |
...
asm_pass3 が `prelude.{text,rodata,data}.bin` + `/sd/u.strip` の
...
| 2 | asm_pass3 に `--emit-bin` モード追加 + reloc detect | ✅ commit 2f6097d |
| 3 | asm_pass3 で `src raw` を memcpy 対応 | ✅ commit 692d8c1 |
| 4 | asm_pass3 で reloc patch 対応 | ✅ commit 749a0f6 |
| 5 | Makefile で host 側 prelude pre-encode 実行 + mtfs ステージ | ✅ commit d7b5e72 |
| 6 | asm_pass2 `--prelude-*` flags + `pico2_bench_idx.sh` 切替 | ✅ commit 40ee22f |
...
- 新パイプライン (`asm_pass2 --add <idx>... --lab-out + asm_pass3
  ...
  - tcheck の 16-bit packed sentinel が strtab > 32 KB で
    ...
    asm_pass2 / asm_pass3 OS-task build が `parse_mem_off__?`
...
各 step は単独で revert 可能。step 3-4 の間は asm_pass3 が中間状態
...
| asm_pass2 | argv (`/path/to/src.s [strip.s]`) + `--load-idx`/`--emit-idx` | 既に file-args（pre-encode で `--emit-bin` 追加） |
| asm_pass3 | stdin (`.lab + 3 × source`) → stdout | `--lab + --src-raw × N + --src-asm × N + --out` |
...
asm_pass3 は **多入力**を扱う必要があるので file-args 必須。tcheck
