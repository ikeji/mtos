...
OS 上でも test_phase7.sh の full split pipeline が通る。Pico 2 の
480 KB kernel arena に **すべての compiler task が余裕で収まる** 状態。
...
| 変数 | 型 | サイズ | 説明 |
|---|---|---:|---|
| `g_lab_names`    | `U8Array(131072)`      | **128 KB** | 全ラベル名を packed byte pool に詰める共有ストレージ (旧 256 KB、2026-04-16 に絞った) |
| `g_labels`       | `LabelArray(4096)`     | **64 KB**  | `Label { name_off, name_len, addr, section }` (4 × i32 = 16 B) × 4096 エントリ (旧 16384、2026-04-16 に絞った) |
| `g_numlab`       | `I32Array(10240)`      | ~40 KB  | 数字ラベル 0:〜9: を digit × 1024 枠で持つ。各 digit 1024 個まで (256 に絞ろうとしたが kernel ビルドで digit 0 が 404 個出て overflow、1024 に戻した) |
...
合計 **~192 KB** ほど。この 2 個だけで ~256 KB の upper bound
を決める。測定で ~260 KB 台なのは、残りは bucket / SourceReader
/ I/O 周辺の細かい allocation。

2026-04-16 の shrink 時に `define_label` / `define_numlab` に
bounds check を追加したので、今後 upper bound を超えた入力が来た
らはっきりとしたエラーメッセージで止まる (以前は silent overflow
で symptom が boot hang や `j 1b` misresolve として現れた)。

#### さらに縮める余地

- `g_lab_names` per-label `kmalloc` 化: packed pool の直接参照を
  ほどいて各 label が自分の name 領域を持つ形に refactor すれば、
  upper bound なしで実 peak だけ払うようになる (測定では self-host
  の label 名総和は ~90 KB)。ただし `name_off` を使っている
  find_label / label_name_eq などの touch 範囲が広くて大改修
- `g_labels` は実測 ~3000 entries なので 2048 まで絞れる可能性
...
だけを 4 KB の `g_out_buf` 経由で直接 stdout に stream emit する
方式に変えた。bss は memsz のみで file bytes 無しなので 4 回目の
pass は無い。
...
|---|---|
| **4.6 MB** | Phase 0 (asm.tc monolith): `g_code = U8Array(4194300)` 固定 |
| **624 KB** | Phase 4 (commit 9e5685f): g_code を filesz = text+rodata+data で動的確保 |
| **445 KB** | Phase 4 + label pool shrink (commit 5098a1e): MAX_LABELS 16384→4096, g_lab_names 256K→128K |
| **280 KB** | Phase 5 (commit 426f51e): **g_code を廃止**。source 3 回 re-read で stream emit |

合計 ~16x 削減。すべての compiler task が Pico 2 kernel arena
(480 KB) に余裕で収まる。
...
3. **pass 0 = text**: section target = 0, source を再スキャン、
   ...
   → sys_write で stdout へ流す。他の section は g_pos だけ
   進める
4. **pass 1 = rodata**: 同じく target = 1
5. **pass 2 = data**: 同じく target = 2

各 pass は `asm_set_src_bytes` で pass 1 が計測しておいた
`src_bytes` を budget として使い、ちょうど 1 copy 分だけ stdin から
消費する。buffered reader の状態は 4 phase を跨いで維持される
(SourceReader を per-pass で作ると内部 buffer 境界で読み過ぎる
ため、共有の `rb_*` reader を自作した)。

emit8 の stream mode 分岐:
    ...
    if g_pass == 2 {
        if g_stream_mode == 1 {
            if g_current_section == g_target_section {
                emit_stream(v);   // → g_out_buf[g_out_pos++]; flush if full
            }
        } else {
            // 旧 buffered path (g_code に書く)
            ...
        }
    }
    g_pos = g_pos + 1;
...
各 section pass の終わりに `asm_flush_section_padding` が
`g_sec_size[target_section]` までゼロを pad し、最後に
`flush_out_buf` で残りを stdout に流す。これで出力バイト数が
ちょうど section_size になり、ELF header の filesz と一致する。

bss section の `.space N` (e.g., 96 MB kernel arena) は target が
どの section でも target != bss なので emit_stream は呼ばれず、
g_pos だけを n 進めるので O(1) で終わる (旧 buffered path では
96 M no-op ループが発生していた)。
...
| 部分 | 推定サイズ |
|---|---:|
| g_lab_names (128 KB) + g_labels (64 KB)    | ~192 KB |
| g_numlab (40 KB)                           | ~40 KB  |
| g_out_buf (4 KB) + g_line_buf (4 KB) + rb_buf (4 KB) | ~12 KB |
