...
※ 実測値は 2026-04-15 に Gen3 tools (compile-gen2.sh で build、
   compiler/runtime.tc の km_dump_peak で自動計測) を qemu-riscv32
   で 14 個の compiler/*.tc と tests/*.tc に対して走らせた結果。
   詳細な per-input テーブルは下の「ステージ別実測表」参照。
...
実測 (2026-04-15 再測定、Gen3 sigscan + qemu-riscv32):
...
#### 実測 (2026-04-15 再測定、Gen3 + qemu-riscv32 / km_dump_peak)
