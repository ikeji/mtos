...
| tcheck    | 717 KB |  80 KB |        | typecheck から派生、symbol を per-item kmalloc、関数ごとに stream check、`-h`/`-s` で .th を複数読む |
...
parse      < a.tc                                    > a.ast
sigscan    < a.ast                                   > a.th
tcheck     -h imports1.th -h imports2.th -s a.th < a.ast > a.tast
```
...
- **`-h <import>.th`** (複数指定可): `(fn:export ...)` のみ register。
  `(fn ...)` (非 export) と global var は無視 (他モジュールから呼べない
  ので不要)
- **`-s <self>.th`**: 全 top-level (fn 全部 / struct 型名 / globals) を
  register

`-h` (import) と `-s` (self) を **明示的に別オプションにする** (positional
な「最後が self」のような暗黙ルールは使わない)。
...
- **sigscan**: AST を stream 読みして top-level だけを書く。関数本体
  (block) は捨てる。AstNode pool は top-level 1 個分 (~500 slot ≈
  18 KB)。
- **tcheck**: 起動時に `-h <file>` / `-s <file>` を順に読み込んで symbol
  table を作る (per-item kmalloc、後述)。stdin (a.ast) を stream 読みして
  関数 1 個分の AST を pool に貯め → check → .tast 出力 → pool reset。
...
struct の合成 fn (constructor / getter / setter / delete / Array まわり)
が **自分の .th** 経由で symbol table に乗るので、fn 数の見積もりは
struct 1 つあたり ~10 fn 増やしておく。例えば自分自身の compiler ファイル
(parse.tc 等) は struct ~10 個 + 直接定義 fn ~100 個 = 200 fn 相当:

- symbol table 本体: 200 fn × ~120 B + 10 struct × ~30 B + 20 global × ~30 B = ~25 KB
