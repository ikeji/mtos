...
| sigscan   | (新規) |  20 KB | extract_sigs.tc 派生、**top-level を全部** (非 export 含む) .th に出す |
| tcheck    | 717 KB |  80 KB | typecheck から派生、symbol を per-item kmalloc、関数ごとに stream check、`-h` で .th を複数読む (最後が self) |
...
```
parse      < a.tc                  > a.ast
sigscan    < a.ast                 > a.th       (a.ast から top-level 全部を抽出)
tcheck     -h imports.th -h a.th < a.ast > a.tast
```
...
参照解決には足りない。具体的に追加が要るもの:

1. **非 export な fn シグネチャ**: 同じファイル内の前方参照 (後で定義
   される非 export 関数を呼ぶ) を解決するため
2. **struct field 一覧**: 自モジュール内では field アクセスを直接書けるので
   field の型情報が要る (他モジュール向けには field は private なので
   要らない、が両用に持っても害はない)
3. **global var 宣言**: 型と (定数なら) 初期値

これを **.th フォーマットの strict superset 拡張** で吸収する:

- 既存の `(fn:export name ...)` はそのまま
- 新規に `(fn name ...)` (annotation なし) を追加 → 非 export 関数
- 既存の `(struct Name)` を `(struct Name (fields (field f1 (type ..)) ...))`
  に拡張
- 新規に `(global name (type ..) (init ..))` を追加

tcheck 側の挙動:

- import の .th を読むとき: `(fn:export ...)` のみ symbol table に登録
  (現行と同じ)。`(fn ...)` は無視。struct fields は無視 (どうせアクセス
  禁止)。
- 自分自身の .th を読むとき: 全部登録
- フラグの渡し方は argv で複数 `-h` を取るのが素直 (`-h imports.th -h
  self.th`)。最後の `-h` が self、それ以外は import 扱い、というのが
  一番簡単。

互換性:

- bootstrap の `extract_sigs.c` / Gen2 の `extract_sigs.tc` を sigscan に
  併合する (or 置き換える)。当面は extract_sigs を残して sigscan を
  別バイナリにし、sigscan の出力する .th が「拡張版」になる
- ホスト側 Gen1 の `make test` パイプラインは extract_sigs (旧フォーマット)
  を使い続ければ良い。Gen2 の compile-gen2.sh だけが sigscan を呼ぶ
- 既存の `golden/tc/*.th` は extract_sigs 由来なので壊れない

#### 内部実装

- **sigscan**: AST を stream 読みして top-level だけを上記 .th に書く。
  関数本体は捨てる。AstNode pool は top-level 1 個分 (~500 slot ≈
  18 KB)。
- **tcheck**: 起動時に `-h <file>` を順に読み込んで symbol table を作る
  (per-item kmalloc、後述)。stdin (a.ast) を stream 読みして関数 1 個分の
  AST を pool に貯め → check → .tast 出力 → pool reset。
...
- symbol table 本体: 100 fn × ~150 B + 30 struct × (~50 B + fields) +
  20 global × ~30 B = ~22 KB
...
- **.th フォーマットの拡張**: 現状の .th は export のみだが、tcheck は
  自分自身の forward 参照解決のために non-export な fn / struct fields /
  global var も必要になる。strict superset 拡張 (`(fn name ...)` /
  `(struct Name (fields ...))` / `(global ...)`) で対応し、tcheck は
  「import の .th からは `:export` のみ register、self の .th からは
  全部 register」とフィルタする。詳細は本計画 §2 の「.th フォーマットの
  拡張」節。`docs/ast_format.md` の .th 説明も更新が必要。
