...
| bc2asm    | 1.4 MB |  40 KB | **per-function emission** (instrs / label_pcs を関数ごとに reset) |
...
- 関数を跨ぐのはシンボル名・string literal index のみ。string literal は
  関数 emit のたびに「未出力分」を `.rodata` に追加 emit する形にすれば、
  literal そのものは関数末端で emit されて bc2asm 自身が長期保持する
  必要はない。
- グローバルなシンボル名 (function 名 / global 変数名) は per-name
  kmalloc。fns / global_vars 配列はそれぞれ 256 entry × handle 8 B
  くらいで足りる (= 2 KB)。

#### 見積もり (1 関数 emit 中の peak)

- instrs バッファ: 16 KB (固定再利用)
- label_pcs: 1 KB (固定再利用)
- 1 関数分 params/locals: 2 KB
- グローバル symbol table: 4 KB
- 入出力バッファ: 8 KB
- 雑多: ~10 KB
- **合計 ≈ 40 KB**
...
- **bc2asm の string literal 集約**: `.rodata` の literal は関数を跨いで
  集まる必要がある (typecheck → codegen で literal index は固まっている)。
  per-function emission では「未出力の literal を関数末端でその場で
  `.rodata` セクションに追加 emit する」形にすれば、bc2asm 内に長期
  state を持たずに済む。出力先は同じ `.rodata` セクションに繋がる
  ので、asm.tc が後でフラット化してくれる。
