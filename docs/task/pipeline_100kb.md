...
| sigscan   | (新規) |  20 KB | **10 KB** | extract_sigs.tc 派生、**top-level を全部** (非 export 含む) .th に出す。block / field / 式は読み捨て (#62 完了) |
| tcheck    | 717 KB |  80 KB | **117〜289 KB** | typecheck から派生、per-top-level reset + per-fn strtab rollback (#51, #63 完了)。fntab stride 47 と vm_run の 2581 nodes が残り bottleneck |
...
実測 (compile-gen2.sh + qemu-riscv32 で `[kmem peak=N live=M]` を
stderr に出力):

| 入力 | km peak | 出力 .th 行数 |
|---|---:|---:|
| compiler/parse.tc       | **10044 B** | 1306 |
| compiler/typecheck.tc   |  9772 B | 1196 |
| compiler/codegen.tc     |  9636 B |  521 |
| compiler/bc2asm.tc      | 10044 B | 1323 |
| compiler/asm.tc         |  9636 B |  737 |
| compiler/runtime.tc     |  9228 B | 1048 |
| compiler/sigscan.tc     |  9372 B |  222 |

入力ファイルにかかわらず ~10 KB で頭打ち。fixed buffers の内訳:
...
#### 実測 (#63 完了後、compile-gen2.sh + qemu-riscv32 / km_dump_peak)

per-top-level pool reset + per-fn strtab rollback + FN_STRIDE=47 (vm_run
の 21 params 対応のため) で:

| 入力 | km peak | nc | sp | fn | vr | tast 行 |
|---|---:|---:|---:|---:|---:|---:|
| compiler/strlib.tc       | 117 KB |   58 |  2521 |  93 |  0 |   386 |
| compiler/string_buffer.tc | 124 KB |  157 |  3407 |  99 |  0 |  1181 |
| compiler/source_reader.tc | 124 KB |  143 |  3847 | 117 |  0 |  1291 |
| compiler/ast_node.tc      | 119 KB |   77 |  3796 | 139 |  1 |  1428 |
| compiler/extract_sigs.tc  | 144 KB |  449 |  8014 | 160 | 13 |  1910 |
| compiler/sigscan.tc       | 145 KB |  461 |  8305 | 164 | 14 |  2405 |
| compiler/runtime.tc       | 129 KB |  221 |  5508 | 159 | 11 |  5457 |
| compiler/typecheck.tc     | 151 KB |  540 | 14303 | 210 | 54 |  7733 |
| compiler/tcheck.tc        | 154 KB |  584 | 14931 | 219 | 60 |  9271 |
| compiler/codegen.tc       | 172 KB |  854 | 13309 | 173 | 34 |  6037 |
| compiler/parse.tc         | 187 KB | 1080 | 19462 | 165 | 67 | 13064 |
| compiler/bc2asm.tc        | 227 KB | 1657 | 21652 | 187 | 47 |  9204 |
| compiler/asm.tc           | 230 KB | 1696 | 21056 | 154 | 34 | 10369 |
| **compiler/bcrun.tc**     | **289 KB** | **2581** | 25109 | 143 | 71 |  9290 |

実装上の固定 buffer:

- `nodes: AstNodeArray(3072)` → refs 12 KB + live ~2581 × ~36 B = ~93 KB
- `strtab: U8Array(32768)` → 32 KB (per-fn rollback で sp は 25 KB 以下)
- `fntab: I32Array(12032)` → 47 KB (256 fns × FN_STRIDE 47)
- `vartab: I32Array(640)` → 2.5 KB
- `atom_buf: U8Array(1024)` → 1 KB
- 入出力 + scratch: ~10 KB

**合計 静的 ≈ 96 KB**、worst case live で +93 KB の AstNode struct =
~190 KB。実測 peak とおおむね一致。

bcrun.tc が突出して大きいのは、`vm_run` という 21 param の bytecode
ディスパッチ関数が 1 つの巨大 switch を持っており、関数本体の AST
ノード数が 2581 と他の倍以上あるため。fntab stride も `vm_run` 1 つの
ために 47 にしている (= 12 wasted i32 per fn × 256 fns = 12 KB の無駄)。

#### 100 KB 目標まで削るには

実測 peak 290 KB → 目標 100 KB には更に 3x 必要。残るボトルネック:

1. **fntab の per-fn kmalloc 化** (-30 KB): FN_STRIDE 47 をやめて、各 fn
   ごとに `kmalloc(20 + np*8)` する。`vm_run` だけが 188 B、ほとんどは
   28 B で済む。fntab を u32 ハンドル列に変えると 256 × 4 = 1 KB。
   accessor (`ft_*`) を全部書き換え必要、中規模リファクタ
2. **AstNode pool の per-fn 動的成長** (-30 KB): いま `AstNodeArray(3072)`
   固定だが、`vm_run` 用に取っているサイズ。リンクトリストや逐次 kmalloc
   構造に変えると ~150 ノード程度に bound できる。これは大規模 refactor
3. **bcrun.tc 側の vm_run 分解** (-50 KB): switch を per-opcode helper
   関数に分解すれば 1 fn あたりのノード数を 1/10 程度にできる。tcheck の
   memory は変えずに peak が下がる

phase 7 OS 上で 256 KB task 枠に収めるには (1) で十分 (fntab -30 KB +
nodes 縮小 -10 KB = -40 KB → ~250 KB peak)。pico2 セルフホスト (= 100 KB
切り) には (1)+(3) または (2) が要る。**現時点では (1) ですらまだ未着手**
で、本タスク #63 は (a) per-top-level pool reset、(b) per-fn strtab
rollback、(c) 固定 buffer の実測値ベース縮小、までで止めている。
