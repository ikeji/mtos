...
#### 使用モジュール: Catalex 系 (AMS1117 LDO + 103 プルアップ)

ピン並び `GND / 3V3 / 5V / CS / MOSI / SCK / MISO / GND` の 8 ピン
モジュール。基板上に **AMS1117** (3.3 V LDO、入力 5 V) と **103**
チップ抵抗 1 個 (= 10 kΩ プルアップ、おそらく CS / MISO のいずれか)
が載っている。他の信号 (CMD/DAT0-3 ≒ MOSI/MISO) のプルアップは
**実装されていない**。SD spec が要求する 10〜100 kΩ プルアップは
RP2350 の内蔵プルアップで代用する (`PADS_BANK0 + 0x04 + pin*4`,
bit 3 = PUE)。GP4 (MISO) と GP7 (MOSI) は内蔵プルアップを有効に
することで補う。
