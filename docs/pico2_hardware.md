...
|---|---|---|---|---|---|
| MISO (SD → MCU) | GP4 | 6 | 左列上から 6 番目 | DO / MISO | SPI0 RX (FUNCSEL=1) |
| CS (active low) | GP5 | 7 | 左列上から 7 番目 | CS | **GPIO 出力** で駆動 (HW CS は使わない) |
| GND | — | 8 | 左列上から 8 番目 | GND | 真ん中の GND を使うと配線が揃う |
| SCK | GP6 | 9 | 左列上から 9 番目 | CLK / SCK | SPI0 SCK (FUNCSEL=1) |
| MOSI (MCU → SD) | GP7 | 10 | 左列上から 10 番目 | DI / MOSI | SPI0 TX (FUNCSEL=1) |
| 電源 3V3 | — | 36 | 右列下から 5 番目 | VCC / VDD | 3V3 OUT (内蔵 LDO、~300 mA まで) |

```
Pico 2 (左下から見上げる)        SD カードモジュール
─────────────────────────        ──────────────────
Pin 6  GP4 ────────────────────  DO   (MISO)
Pin 7  GP5 ────────────────────  CS
Pin 8  GND ────────────────────  GND
...
- **3.3 V ロジックのモジュールを使う**。一部の Arduino 用モジュール
  (Catalex 等) は 5 V 入力レベル変換が入っているが、VCC を 3V3 に
  繋げば多くは 3.3 V でも動く。ただし変換 IC によっては 5 V 必須の
  ものもあるので、購入時に **3V3 動作可** の表示を確認
- SD カードは突入電流が大きい (短時間で 200〜300 mA)。Pico 2 の
  3V3 OUT (内蔵 LDO) は 300 mA まで供給できるのでギリギリ足りるが、
  容量が厳しい SD カードでは外部 3.3 V レギュレータを足したほうが
  安定する
- SPI mode の SD カード仕様では **CMD / DAT0-3 に 10〜100 kΩ プルアップ**
  が必須。多くのモジュールは基板上に実装済みだが、念のためテスタで
  確認。最低限 MISO (DO) には外付けプルアップを足すと安定する
- SCK は最初 100〜400 kHz で初期化、SDHC 認識後に 12 MHz (= 本機の
  最大、`clk_peri` が XOSC のため) まで上げる
