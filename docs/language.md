...
- 可変長引数なし
- 関数ポインタなし
- **グローバル変数は値型のみ**（参照型はローカル変数・関数引数・戻り値のみ）
  - 参照型をグローバルに置くとランタイム初期化が必要になるため禁止
  - グローバルな状態が必要な場合は値型（`i32`, `u32`, `bool` 等）で表現する
...
```
newI32Array(size: u32) -> I32Array    // 確保
...
他の型も同様（`newU8Array`, `get(arr: U8Array, ...)` など）。
`String` も参照型なので `StringArray` も組み込みで提供される。

```
newStringArray(size: u32) -> StringArray
...
```
newPoint(x: i32, y: i32) -> Point   // 確保・初期化
delete(p: Point)                     // 解放
...
```
newPointArray(size: u32) -> PointArray
...
var arr: PointArray = newPointArray(8);
set(arr, 0, newPoint(1, 2));
...
// 変数宣言
var x: i32 = 10;
var p: Point = newPoint(1, 2);
var buf: U8Array = newU8Array(64);
...
| `newXxxArray(size)` | 配列の確保 |
| `len(arr)` | 配列の長さ |
| `get(arr, idx)` | 配列要素の取得 |
| `set(arr, idx, val)` | 配列要素の設定 |
| `delete(ref)` | 配列・構造体の解放 |
| `newXxx(...)` | 構造体の確保（struct宣言で自動生成） |
