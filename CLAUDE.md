...
docs/       仕様・設計ドキュメント
  ...
  task/              タスク計画・デバッグノート
kernel/     カーネル（プリエンプティブマルチタスク、qemu virt で動作）
  kernel.tc           カーネル本体（スケジューラ、trap handler）
  crt0_virt.s         virt 向け crt0（trap entry/exit、ecall handler）
  crt0_data.s         共通 BSS（__arena、trap frame、kern save、switch frame）
  build_virt.sh       タスクビルド → bin2s → カーネルビルド一括実行
  bin2s.sh            raw バイナリ → .s データ変換
  tasks/              ゲストタスク
    task_crt0.s       タスク用 crt0（ecall syscall）
    task_data.s       タスク用最小 BSS
    hello/hello.tc    タスク1（"A" 出力）
    hello2/hello2.tc  タスク2（"B" 出力）
os/         OSカーネル（旧、kernel/ に移行）
```
...
# → タスク (hello.tc, hello2.tc) を raw バイナリにコンパイル
# → bin2s.sh で .rodata に変換
# → kernel.tc + タスクデータ + crt0_virt.s をビルド
...
タスクは ecall で syscall を発行 (Linux 互換 ABI: a7=64 write, a7=93 exit)。
カーネルの ecall handler がアセンブリでディスパッチする。
タイマ割り込みで TC の trap_handler がラウンドロビンスケジューリングを実行。
詳細は `docs/task/kernel_design.md` を参照。
