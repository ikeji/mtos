...
`docs/roadmap.md` 参照。フェーズ6 (ユーザランド基盤) まで完了:
mtfs 上の `/bin/*` を VFS ローダ経由で起動でき、`/bin/sh` が
`sys_spawn` + `sys_wait` でコマンドをループ実行する。次は
フェーズ7 (OS 上でコンパイラを動かす) か、フェーズ2.5 残り
(PSRAM / Pico 2 セルフホスト)、あるいは `docs/problem.md` の
bug 消化 (u32 比較 / u32 除算) のいずれか。
