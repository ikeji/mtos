...
- stdin 先頭ディレクティブ:
  ...
  - `; gp 0xHHHHHHHH` — gp の論理アドレスを設定。非 0 のとき、`.data`/`.bss`
    セクションにあるラベルへの `la` を `auipc rd, 0` + `addi rd, gp, offset`
    (offset は `(label - data_base - 0x800)`、12-bit signed 範囲) に展開する。
    範囲外はコンパイルエラーとしてラベル名付きで stderr に出して exit。

## compile-gen2.sh の環境変数
デフォルトは Linux ELF 用の crt0 だが、以下を設定すると別ターゲット
（qemu virt, Pico 2 等）向けにビルドできる:
- `CRT0` — `.s` ファイルパス。デフォルト `bootstrap/crt0_tc.s`
- `CRT0_DATA` — `.s` ファイルパス。デフォルト `bootstrap/crt0_tc_data.s`
