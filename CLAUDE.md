...
`docs/roadmap.md` 参照。フェーズ6 (ユーザランド基盤) 完了に加え、
`docs/problem.md` のレビューを一通り済ませて以下が解消済:

- u32 比較 / u32 除算 / 16 進リテラル top bit 等のコンパイラバグ
- K1 (task 画像 leak) + スケジューラを struct Task に畳み込み
- typecheck / bounds check エラーに関数名 + 直前 comment を付与
- asm.tc エラーに行番号 + 元行
- stderr ノイズを取って compile-gen*.sh の `2>/dev/null` を全て撤廃
- R1-R8 refactor 全消化: `compiler/ast_node.tc` 新設で
  codegen/typecheck/extract_sigs の AST ノードプールを共有、
  `streq` / `cmp` / `emit_string` / `emit_nl` ヘルパで冗長コード縮小、
  `struct BcFunc` / `struct Label` / `struct Task` で並列配列撲滅、
  bc2asm を SourceReader ストリーミングに移行 (524 KB→4 KB)
- `make test`: 132 passed / ~50s (58s から短縮)

次は フェーズ7 (OS 上でコンパイラを動かす) か、フェーズ2.5 残り
(PSRAM / Pico 2 セルフホスト。asm.tc の ~9 MB 静的バッファが主な
残ボトルネック) のいずれか。
...
ビルドフロー (両ターゲット共通):
1. ゲストタスク (target 別 TASKS リスト — virt は hello/hello2/catfile/sh、
   pico2 は hello/hello2/catfile/launcher) を raw バイナリにコンパイル
2. `tools/mkfs.py` で `/bin/<task>` + `/hello.txt` を含む mtfs イメージを生成
3. virt は `--disk-out` でイメージを出力し、qemu の `-drive` から読む。
   pico2 は `bin2s.sh _mtfs_image` で .rodata に埋め込み、XIP 経由で読む
4. platform_*.s + trap_common.s + crt0_*_data.s + kernel*.tc (+pico2 は mtfs image)
   を asm.tc で結合してリンク
5. 起動時にカーネルの `loader.tc` (`load_task`) が `/bin/*` を VFS で開き、
