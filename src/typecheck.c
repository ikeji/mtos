...
static void collect_struct(TypeEnv *e, AstNode *node) {
    ...
    for (int i = 0; i < sd->nfields; i++) {
        const char *ft = sd->fields[i].type;
        const char *fn = sd->fields[i].name;
        /* getter: fieldName(s: StructName) -> ft */
        const char *getter_args[] = {sname};
        register_fn(e, fn, getter_args, 1, ft);
        /* setter: fieldName(s: StructName, val: ft) */
        const char *setter_args[] = {sname, ft};
        register_fn(e, fn, setter_args, 2, "void");
    }

    /* array type: StructNameArray */
    char arr_type[128];
