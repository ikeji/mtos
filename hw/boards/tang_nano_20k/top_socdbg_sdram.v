...
module top_socdbg_sdram (
    ...
    soc #(.USE_SDRAM(1), .CLK_HZ(27_000_000), .BAUD(921_600), .RESET_PC(32'h0), .ROM_WORDS(2048), .ROM_INIT("build/bootrom.hex")) u_soc (
        ...
        .sdram_addr(O_sdram_addr), .sdram_ba(O_sdram_ba), .sdram_dqm(O_sdram_dqm), .sdram_dq(IO_sdram_dq), .gpio_out(), .gpio_oe(), .gpio_in(48'b0), .spi_sck(), .spi_cs_n(), .spi_mosi(), .spi_miso(1'b1), .sd_sck(), .sd_cs_n(), .sd_mosi(), .sd_miso(1'b1), .lcd_sck(), .lcd_mosi(),
