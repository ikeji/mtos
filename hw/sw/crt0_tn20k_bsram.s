# hw/sw/crt0_tn20k_bsram.s — crt0 for the hw/ SoC with only 32 KB of BSRAM
# (Phase 1/2 bring-up before SDRAM): sp at 0x80008000, 2 KB kmalloc arena.
# Generated from crt0_tn20k.s (derived from compiler/tests/virt_crt0.s;
...
_start:
    ...
    call main
    # Explicit jump (not just fall-through): asm_pass2's dead-strip is
    # symbol-based and only follows references, so without this `_park`
    # is stripped whenever do_exit is unused and main's return would
    # fall into _trap_entry (docs/problem.md #46).
    j    _park
